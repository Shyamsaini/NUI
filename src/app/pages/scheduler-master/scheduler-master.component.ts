...
export class SchedulerMasterComponent implements OnInit {    
  ...
  intervals: number[] = [];
  apiUrls: any[] = []; // This will hold your API URLs
  ...
  constructor(private fb: FormBuilder, private schedulerService: SchedulerMasterService,private authService: AuthService) {}
  ...
  loadStateOptions(): void {
    //If you're getting state from an API:
    //this.selectedState = 'Active';   
    const stateCode = localStorage.getItem('stateCode') ?? '';
    this.authService.getStates().subscribe((data: any[]) => {
     const StateName = data.filter(state => state.StateCode === stateCode);
    this.selectedState=StateName[0].stateName;
  }
  );  
}
    ...
    this.schedulerService.getSchedulers().subscribe({
      next: (response: any) => {        
        ...
        if (response.isSuccess && response.data) {
          this.schedulerRecords = response.data;
          
          // Extract and transform processApiUrl data
          this.apiUrls = response.data
            .filter((record: any) => record.processApiUrl) // Filter records with processApiUrl
            .map((record: any) => ({
              value: record.processApiUrl,
              display: record.processApiUrl
            }));
