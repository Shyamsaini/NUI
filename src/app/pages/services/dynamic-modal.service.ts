...
export class DynamicModalService {
  ...
  // }
  loadMedleparInformation(data:any){
    return this.utilityService.LoadMedleprInformation(data);
  }

  loadFslInformation(fslId: any){
    return this.utilityService.LoadFslInformation({fslId: fslId.fulldispno,psCode:"6142030"});
  }
  ...
  loadProsecutionInformation({ prosecutionId, psCode }: { prosecutionId: string, psCode: string }) {
    return this.utilityService.LoadProsecutionInformation({ eProcId: prosecutionId, psCode });
  }
   loadPrisonInformation(data: any){
    debugger;
    return this.utilityService.LoadPrisonerInformation({PrisonerId: String(data.pid_no) ,StateCode:data.state_code});
  }
