...
export class FslReportComponent implements OnInit {
  ...
  onSearch() {
    const formFromDate = this.form.controls["FromDate"].value;
    const formToDate = this.form.controls["toDate"].value;

    this.fromDate = formFromDate ? this.formatDateToYMD(new Date(formFromDate)) : '';
    this.toDate = formToDate ? this.formatDateToYMD(new Date(formToDate)) : '';
