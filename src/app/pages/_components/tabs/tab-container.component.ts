...
export class TabContainerComponent {
  ...
  selectTab(index: number, psCode: any) {
  ...
  loadComponent(index: number) {
    ...
    const psCode=this.otherPillarData;
    const ps_Code = psCode?.firregnum.slice(0, 7);
    if (tab.component.name == "_OtherPillarsComponent"){
      if(this.otherPillarData === undefined || this.otherPillarData.firregnum != tab.data.firNumber){
        ...
        this.loadOtherPillarData(tab.data).subscribe(response => {
          if (response.isSuccess) {
            this.psCode=tab.data.policeCode;
            this.otherPillarData = response.data;
            this.loadDynamicComponent(index, this.otherPillarData,ps_Code);
      ...
      if(cnrInfo){
        this.loadDynamicComponent(index, cnrInfo,ps_Code);
        this.loaderService.hide();
      }
      else{
        this.loadCaseInformation(tab.data).subscribe(response=>{
          if(response.isSuccess){
            ...
            this.loadDynamicComponent(index, data,ps_Code);
            this.loaderService.hide();
          }
          else{
      ...
      if(fslInfo){
        this.loadDynamicComponent(index, fslInfo,ps_Code);
        this.loaderService.hide();
      }
      else{
      ...
      if(prosecutionInfo){
        this.loadDynamicComponent(index, prosecutionInfo,ps_Code);
        this.loaderService.hide();
      }
      else{
        this.loadProsecutionInformation(tab.data,ps_Code).subscribe(response=>{
          if(response.isSuccess){
            const data = JSON.parse(response.data);
            this.prosecutionInfos.push(data.data);
            this.loadDynamicComponent(index, data.data,ps_Code);
            this.loaderService.hide();
          }
          else{
            this.utilityService.ShowErrorPopup(response.message);
      ...
      if(prsionerInfo){
        this.loadDynamicComponent(index, prsionerInfo,ps_Code);
        this.loaderService.hide();
      }
      else{
      ...
      if(miniStatement){
        this.loadDynamicComponent(index, miniStatement,ps_Code);
        this.loaderService.hide();
      }
      else{
      ...
      if(medleprInfo){
        this.loadDynamicComponent(index, medleprInfo,ps_Code);
        this.loaderService.hide();
      }
      else{
  ...
  loadDynamicComponent(index: number, componentData: any, ps_Code: string | undefined){
    ...
    if(tab.component.name == "_OtherPillarsComponent"){
      ...
      componentRef.instance.loadFslInfo(componentData,ps_Code);
    }
    else if(tab.component.name == "_ProsecutionComponent"){
      componentRef.instance.loadProsecutionInfo(componentData,ps_Code);
    }
    else if(tab.component.name == "_PrisonComponent"){
  ...
  loadProsecutionInformation(eProcId: string, ps_Code: string | undefined){
    return this.utilityService.LoadProsecutionInformation({eProcId: eProcId,ps_Code:ps_Code});
  }
  loadMedleprInformation(data:any){
    return this.utilityService.LoadMedleprInformation(data);
  }
