...
interface majorHeadsCodesOptions { 
  text: string;
  value: string;
}
@Component({
...
export class IOdashboardComponent implements OnInit {
  ...
  DashboardType: string = 'MyDashboard';
  actTypeOptions = [
    { label: 'All', value: 'all' },
    { label: 'NCL', value: 'NCL' },
    { label: 'BNS', value: 'BNS' },
  ...
  ListMajorHeads: { value: any; text: any; }[] | [] = [];
  selectedMajorHeadsType: any[] = [];
  majorHeadsCodes : any[] = [];
  isFirFirstLoad: boolean = true;
  constructor(
    ...
    private apiservice: FirReportService
  ) {
    ...
    this.form = this.formBuilder.group(
      {
        ...
        ToDate: [''],
        districtCodes: [''],
        MajorHeadsType: [''], // Added for Major Heads filter
  ...
  toggleState() {
    ...
    this.timependencies = []
    this.toDate = '';
    this.majorHeadsCodes = [];
    this.selectedMajorHeadsType = [];
    this.form.controls["actType"].setValue('all');
    this.form.controls["MajorHeadsType"].setValue('');
  ...
  LoadDataForCount() {
    ...
    this.IodashboardService.LoadFirCounts(this.districtsCodes, this.policeStationsCodes, this.fromDate, this.toDate, this.DashboardType, this.actType || '' , this.majorHeadsCodes).subscribe({
      next: (response: any) => {
...
onPsSelectAll(items: any[]) {
  ...
  onMajorHeadsSelect(item: any) {
   const selectedItems = this.form.get('MajorHeadsType')?.value || [];
   this.selectedMajorHeadsType = selectedItems.map((ps: any) => ps.text);
   }

   onMajorHeadsSelectAll(items: any[]) {
    this.selectedMajorHeadsType = items.map(item => item.text);
   }
...
get trimmedMajorHeads(): string {
  const majorHeads = this.selectedMajorHeadsType.join(', ');
  if (!majorHeads) return '';
  return this.readMore? majorHeads : majorHeads.length > 100? majorHeads.slice(0, 100) + '...': majorHeads;
}
toggleReadMore() {
...
toggleReadMoreDistricts() {
  ...
  onSearch() {
    ...
    const majorHeadsCodes: majorHeadsCodesOptions[] = this.form.controls["MajorHeadsType"].value || [];
    const majorCodeValues = majorHeadsCodes.length > 0 ? majorHeadsCodes.map((item: majorHeadsCodesOptions) => Number(item.value)) : [];
    this.majorHeadsCodes = majorCodeValues;
    ...
    if (mainType == 'Pending') {
      ...
      if (this.actType == 'all') {
        this.actType = '';
          ...
          //Major Heads filter
      this.IodashboardService.LoadTimePendency(this.CountFromDate, this.CountToDate, this.fromDate, this.toDate, this.policeStationsCodes, this.districtsCodes, this.mainType, this.DashboardType, this.actType || '', this.majorHeadsCodes).subscribe({
        next: (response: any) => {
          if (response.isSuccess && response.data && response.data.length > 0) {
  ...
  LoadFirListPagination(event: TableLazyLoadEvent) {  
   ...
   // this.currentLast = this.pageSize > this.FilteredRecordsCount ? this.FilteredRecordsCount : this.pageSize;
    ...
    if(this.isFirFirstLoad)
    {
      this.isFirFirstLoad = false;
      return
    }
    this.loaderService.show();
    this.DataFilter = [];
  ...
  //   this.currentLast = first + this.pageSize > this.FilteredRecordsCount ? this.FilteredRecordsCount : first + this.pageSize;
    ...
    this.IodashboardService.LoadFirList(this.fromDateTimePendecny, this.toDateTimePendecny, this.fromDate, this.toDate, this.districtsCodes, this.policeStationsCodes, this.subType, this.searchTerm, this.pageNumber, this.pageSize, this.sortBy, this.DashboardType, this.actType || '',this.majorHeadsCodes).subscribe({
      next: (response: any) => {
  ...
  CallFirListApi(fDate: string, tDate: string) {
    ...
    this.IodashboardService.LoadFirList(fDate, tDate, this.fromDate, this.toDate, this.districtsCodes, this.policeStationsCodes, this.subType, this.searchTerm, this.pageNumber, this.pageSize, this.sortBy, this.DashboardType, this.actType || '',this.majorHeadsCodes).subscribe({
      next: (response: any) => {

        if (response.isSuccess && response.data && response.data.length > 0) {
          this.ListTable = true;
          if (this.typeOfFir == 'Pending') {
            this.isTotalSecondCount = true;
          }
          else{
            this.isTotalSecondCount = false;
  ...
  popDataSetLoad(): void {
    ...
    this.IodashboardService.LoadFirListCount(this.fromDateTimePendecny, this.toDateTimePendecny, this.fromDate, this.toDate, this.districtsCodes, this.policeStationsCodes, this.subType, this.searchTerm, this.pageNumber, this.pageSize, this.sortBy, this.DashboardType, this.actType || '',this.majorHeadsCodes).subscribe({
      next: (response: any) => {
        if (response.isSuccess && response.data && response.data.length > 0) {
  ...
  loadPillarData(firNumber: string, firRegDate: string, firShortNumber: string, policeCode: string): void {
     ...
     localStorage.setItem('FIRNO', JSON.stringify(firNumber));
    localStorage.setItem('PS_CD', JSON.stringify(policeCode));
